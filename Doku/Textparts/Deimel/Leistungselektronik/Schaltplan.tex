...
\begin{circuitikz}[loops/.style={circuitikz/inductors/coils=#1}]]
    ...
    \foreach \x/\y/\ii in {\offxmotor/\offymotor/1, \offxmotor/\offymh/2}{
        ...
        \draw[-latex reversed, double] (\x-4, \y+.2) -- ++(-.4,0) node[left] {\scriptsize PWM-M\ii};
        \draw[-latex, double] (\x-4, \y-.2) -- ++(-.4,0) node[left] {\scriptsize Serial-M\ii};
    }
    ...
    % J1 Mikroswitsches
    \draw (\platineX+2-1,\platineY-5) -- ++(0,.5) -- ++(1,0) -- ++(0,-.5);
    \draw (\platineX+2.5-1,\platineY-5+.5) node[above] {\small J1};

    \draw (\platineX+2.75-1,\platineY-4.75) to[short, *-] ++(0,-.75) to[nos] ++(1.5,0) to[nos] ++(1.5,0) -- ++(.25,0) -- ++(0,-.75) -- (\platineX+2.25-1,\platineY-4.75-.75-.75) to[short, -*] (\platineX+2.25-1,\platineY-4.75);

    \draw (\platineX+2.75+1.5-1,\platineY-4.75-.75) node[below] {\scriptsize Ladedeckel-Endschalter};
