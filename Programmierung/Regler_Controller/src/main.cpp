...
void loop()
{
  ...
  { //check for new control data
    ...
    {                                //check if correct message was received and if its length is correct
  ...
  if (micros() - last_data_received >= MAX_TIME_WITHOUT_UPDATE)
  { //stop the motor if no data from CAN-Bus was received for 400 milliseconds
    OCR1AH = 0b01;
    OCR1AL = MIN_PWM_VALUE;
  }
